...
plot_dirJPG <-
function(x,y,cycle,select=0,xlim,ylim,xlab="X",ylab="Y",imgs=NULL,
                   roi=c(xlim[2]/2,ylim[2]/2),r=ylim[2]/10){
  require(interp)
  if(!is.null(imgs)){require(jpeg)}
  #x11()
...
rasterImage(imgs[i],xleft=xlim[1], xright=xlim[2],  ybottom=ylim[1], ytop=ylim[2]
)
